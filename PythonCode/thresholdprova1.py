...
val_loader = DataLoader(NIHChestDatasetXRV(pd.read_csv(val_csv), transform), batch_size=BATCH_SIZE, shuffle=False, num_workers=4, collate_fn=collate_fn)
test_loader = DataLoader(NIHChestDatasetXRV(pd.read_csv(test_csv), transform), batch_size=BATCH_SIZE, shuffle=False, num_workers=4, collate_fn=collate_fn)

# A. Calcolo Soglie
print("Calcolo soglie su Validation...")
...
test_preds_bin = (np.vstack(test_preds) >= np.array(best_thresholds)).astype(int)
print(classification_report(np.vstack(test_targets), test_preds_bin, target_names=classes, zero_division=0))