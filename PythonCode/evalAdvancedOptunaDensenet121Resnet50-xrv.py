...
# =====================================================
# OTTIMIZZAZIONE BAYESIANA CONGIUNTA (OPTUNA)
# =====================================================
print("\n" + "="*60)
print("AVVIO OTTIMIZZAZIONE BAYESIANA CONGIUNTA (OPTUNA) PER CLASSE")
print("="*60)
...
for j, c in enumerate(classes):
    ...
    def objective(trial):
        # Spazio continuo per il peso Alfa e per la Soglia clinica
        ...
        try:
            ...
            # Massimizziamo la combinazione lineare di AUC e F1 per bilanciare calibrazione e confidenza
            return auc_val + 0.5 * f1_val
        except ValueError:
            return 0.0

    # Studio Optuna guidato dal campionatore TPE (Tree-structured Parzen Estimator)
    study = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=42))
    study.optimize(objective, n_trials=300)
    
    best_params = study.best_params
    a_opt = float(best_params["alpha"])
    t_opt = float(best_params["threshold"])
    
    mixed_prob_val_opt = a_opt * val_v4[:, j] + (1.0 - a_opt) * val_v5[:, j]
    try:
    ...
    print(f"-> {c:<20} | Opt Alpha (v4): {a_opt:.3f} | Opt Thresh: {t_opt:.3f} | Val AUC congiunta: {auc_val_final:.4f}")

# =====================================================
# EVALUATION FINALE SUL TEST SET
# =====================================================
print("\n" + "="*60)
print("EVALUATION FINALE SUL TEST SET CON PARAMETRI OPTUNA")
print("="*60)
...
print(f"➔ PRECEDENTE ROC-AUC (Grid Search Sequenziale):     0.8512")
print(f"➔ NUOVO MACRO AVERAGE ROC-AUC CONGIUNTO OPTUNA:   {optimized_macro_auc:.4f}")
print("-"*60)

# Esportazione Checkpoint Finale
...
output_json_path = "checkpoints/hyperoptimized_ensemble_config.json"
os.makedirs("checkpoints", exist_ok=True)
with open(output_json_path, "w") as f:
    json.dump(hyperparameters_output, f, indent=4)
print(f"\n[FINISH] Nuova configurazione Bayesiana salvata in: {output_json_path} 🚀")