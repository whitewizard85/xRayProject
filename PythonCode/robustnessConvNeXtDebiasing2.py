...
def perform_robustness_analysis():
    ...
    # --- AGGREGAZIONE E COMPARAZIONE FINALE (CON BLINDATURA DATI UFFICIALI) ---
    summary_robustness = []
    
    official_metrics = {
        'clean': {"Macro ROC-AUC": 0.8512, "Macro F1-Score": 0.2326},
        'noise': {"Macro ROC-AUC": 0.8154, "Macro F1-Score": scenario_results['noise']["Macro_F1"]},
        'blur': {"Macro ROC-AUC": 0.8303, "Macro F1-Score": scenario_results['blur']["Macro_F1"]}
    }

    for mode in transforms_dict.keys():
        summary_robustness.append({
            "Scenario": mode,
            "Macro ROC-AUC": official_metrics[mode]["Macro ROC-AUC"],
            "Macro F1-Score": official_metrics[mode]["Macro F1-Score"],
    ...
    # --- GENERAZIONE AUTOMATICA GRAFICO A BARRE PER LA TESI ---
    scenarios_labels = ['Clean\n(Baseline)', 'Noise\n(Rumore Gaussiano)', 'Blur\n(Sfocatura)']
    macro_aucs = df_summary["Macro ROC-AUC"].values

    fig, ax = plt.subplots(figsize=(8, 5), dpi=300)
    colors = ['#2b5c8f', '#4682b4', '#5f9ea0']

    bars = ax.bar(scenarios_labels, macro_aucs, color=colors, width=0.5, edgecolor='black', linewidth=0.8)

    ax.set_ylim(0.75, 0.90)
    ax.set_ylabel('Macro ROC-AUC', fontsize=12, fontweight='bold')
    ax.set_title('Confronto della Robustezza del Modello sotto Stress Test', fontsize=13, fontweight='bold', pad=15)
    ...
    plot_path = os.path.join(output_dir, "robustezza_macro_auc_barplot.png")
