...
output_error_csv = "checkpoints/error_analysis_v4_xrv.csv"
...
model.eval()
print(f"\nModello v4 (XRV + 512px) caricato con successo per l'Error Analysis! ✔")
...
with torch.no_grad():
    for images, labels, names in tqdm(test_loader):
        ...
        # Coerenza con autocast a precisione mista
        with torch.cuda.amp.autocast():
            outputs = model(images)
            
        probs = torch.sigmoid(outputs).cpu().numpy().astype(np.float32) # Convertito per evitare float16
...
print("\n" + "="*70)
print("--- TOP 5 PEGGIORI FALSI NEGATIVI (Malattie mancate clamorosamente) ---")
print("="*70)
